...
se_1 = pd.DataFrame(se1[1:],columns=['Query_id','Doc_ID','Rank','Score']) #convert the SE1 into a dataframe
se2 = readfile("Results_from_SE2.tsv")
se_2 = pd.DataFrame(se2[1:],columns=['Query_id','Doc_ID','Rank','Score']) #convert the SE2 into a dataframe
se3 = readfile("Results_from_SE3.tsv")
se_3 = pd.DataFrame(se3[1:],columns=['Query_id','Doc_ID','Rank','Score']) #convert the SE3 into a dataframe
# GT - from list to df
...
print('nDCG for SE1: ', round(ndcg1,4))
ndcg2 = n_dcg(se_2,ground_truth,k)
print('nDCG for SE2: ', round(ndcg2,4))
ndcg3 = n_dcg(se_2,ground_truth,k)
print('nDCG for SE3: ', round(ndcg3,4))